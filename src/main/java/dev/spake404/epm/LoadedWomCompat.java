...
final class LoadedWomCompat implements WomCompat {
	...
	public boolean consumeNaturalSprinterStep(PlayerPatch<?> playerPatch) {
		...
		SkillContainer naturalSprinter = findNaturalSprinter(playerPatch);
		Skill skill = naturalSprinter == null ? null : naturalSprinter.getSkill();
		if (skill == null) {
			return false;
		}

		float consumption = naturalSprinterStepConsumption(naturalSprinter);
		if (!playerPatch.hasStamina(consumption)) {
			return false;
		}

		try {
			return playerPatch.consumeForSkill(skill, Skill.Resource.STAMINA, consumption);
	...
	private float naturalSprinterStepConsumption(SkillContainer naturalSprinter) {
		Float configuredConsumption = getData(naturalSprinter.getDataManager(), key(WOMSkillDataKeys.CONSUMPTION_VALUE));
		if (configuredConsumption == null || configuredConsumption.floatValue() <= 0.0F) {
			return NATURAL_SPRINTER_STEP_FALLBACK_CONSUMPTION;
		}
		return configuredConsumption.floatValue();
	}
