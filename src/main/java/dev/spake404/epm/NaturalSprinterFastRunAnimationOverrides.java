...
public final class NaturalSprinterFastRunAnimationOverrides {
	...
	static RuleData select(ResourceLocation itemId, String weaponType) {
	...
	static boolean isConfiguredRunAnimation(AssetAccessor<?> animation) {
		if (animation == null) {
			return false;
		}
	...
	static record RunPoseSettings(boolean enabled, float scale, float blendTicks) {
		static final RunPoseSettings DEFAULT = new RunPoseSettings(true, 0.4F, 3.0F);
		static final RunPoseSettings DISABLED = new RunPoseSettings(false, 0.0F, 3.0F);
	...
	static final class RuleData {
		...
		static RuleData decode(FriendlyByteBuf buffer) {
