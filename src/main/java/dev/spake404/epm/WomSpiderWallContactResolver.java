...
final class WomSpiderWallContactResolver {
	private WomSpiderWallContactResolver() {
	}

	static Direction detectAdjacentWallDirection(Player player) {
		if (player == null) {
			return null;
		}
	...
	static Direction detectAdjacentWallDirection(Player player, float yaw) {
		if (player == null) {
			return null;
		}
	...
	static boolean hasAdjacentWallDirection(Player player, Direction direction) {
	...
	static float wallFacingYaw(Player player, Direction wallDirection) {
	...
	static Vec3 rightSideDirection(Direction wallDirection) {
		Vec3 normal = wallNormal(wallDirection);
		return new Vec3(-normal.z(), 0.0D, normal.x());
	}

	static Vec3 wallNormalDirection(Direction wallDirection) {
