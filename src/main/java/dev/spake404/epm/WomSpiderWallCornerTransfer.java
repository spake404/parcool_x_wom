...
final class WomSpiderWallCornerTransfer {
	private static final double CORNER_TRANSFER_DOT = 0.65D;

	private WomSpiderWallCornerTransfer() {
	}

	static Direction findAdjacentCornerWall(Player player, Direction activeWall, int womSide) {
	...
	static boolean canTransferTo(Direction activeWall, Vec3 runDirection, Direction candidateWall) {
	...
	static Vec3 targetRunDirection(Direction wallDirection, int womSide) {
		Vec3 wallRight = WomSpiderWallContactResolver.rightSideDirection(wallDirection);
		return womSide < 0 ? wallRight : wallRight.reverse();
	}

	static Direction directionFromWallVector(Vec3 wallDirection) {
	...
	static Vec3 wallVector(Direction direction) {
