...
public final class CrawlAnimationHandler {
	...
	private static final WeakHashMap<Player, Phase> NEXT_ENTRY_MOVES = new WeakHashMap<>();
	...
	public static void chooseAnimation(UpdatePlayerMotionEvent.BaseLayer event) {
		...
		if (state == null || state.phase == Phase.EXIT) {
			state = new CrawlState(claimEntryMove(player));
	...
	private static boolean isSliding(Player player) {
		Parkourability parkourability = Parkourability.get(player);
		return parkourability != null && parkourability.get(Slide.class).isDoing();
	}
	...
	private static Phase claimEntryMove(Player player) {
		Phase selected = NEXT_ENTRY_MOVES.get(player);
		if (selected == null) {
			selected = player.getRandom().nextBoolean() ? Phase.MOVE_LEFT : Phase.MOVE_RIGHT;
		}

		NEXT_ENTRY_MOVES.put(player, selected == Phase.MOVE_LEFT ? Phase.MOVE_RIGHT : Phase.MOVE_LEFT);
		return selected;
	}
	...
	private static final class CrawlState {
		...
		private Phase nextMove;

		private CrawlState(Phase nextMove) {
