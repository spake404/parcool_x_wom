...
public final class ParCoolRightClickActionPriority {
	...
	public static String blockingActionForSpiderWallSlide(Player player) {
		return blockingAction(player, PriorityCheck.SPIDER_WALL_SLIDE);
	}

	private static String blockingAction(Player player, PriorityCheck check) {
		if (player == null) {
			return null;
		}
	...
	private enum PriorityCheck {
		SPIDER_WALL_SLIDE(true);

		private final boolean probeStarts;

		PriorityCheck(boolean probeStarts) {
			this.probeStarts = probeStarts;
		}

		private boolean probeStarts() {
			return probeStarts;
		}
	}
