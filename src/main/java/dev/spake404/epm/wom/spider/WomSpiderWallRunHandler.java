...
public final class WomSpiderWallRunHandler {
	...
	private static final double WALL_BACKFLIP_AWAY_SPEED = 0.32D;
	private static final double WALL_BACKFLIP_RUN_SPEED = 0.08D;
	private static final double WALL_BACKFLIP_PRESERVED_MOMENTUM = 0.35D;
	private static final double WALL_BACKFLIP_MAX_HORIZONTAL_SPEED = 0.42D;
	private static final double WALL_BACKFLIP_VERTICAL_SPEED = 0.2D;
	...
	public static void tick(Player player) {
		...
		if (isParCoolWallJumpActive(player) || isEpicParCoolWallJumpAnimation(playerPatch)) {
			if (tryTakeOverParCoolWallJump(player, playerPatch, wallRunKeyDown, "tick")) {
				return;
			}
	...
	public static boolean handleMovementInput(MovementInputEvent event) {
		...
		if (isParCoolWallJumpActive(player) || isEpicParCoolWallJumpAnimation(playerPatch)) {
			if (tryTakeOverParCoolWallJump(player, playerPatch, wallRunKeyDown, "input")) {
				return true;
			}
	...
	public static Direction activeWallDirection(Player player) {
		WallRunState state = player == null ? null : ACTIVE_WALL_RUNS.get(player);
		if (state == null) {
			return null;
		}
	...
	public static void releaseForParCoolWallJump(Player player, PlayerPatch<?> playerPatch, String reason) {
		if (player == null || playerPatch == null || !isWallRunActive(player)) {
			return;
		}
		...
		logWallRunState(
				"handoff_stop",
				reason == null ? "parcool_walljump_started" : reason,
				player,
				playerPatch,
				ACTIVE_WALL_RUNS.get(player),
				wallRunKeyDown);
		suspendForParCoolWallJump(player, playerPatch, wallRunKeyDown);
	}

	private static boolean tryTakeOverParCoolWallJump(Player player, PlayerPatch<?> playerPatch, boolean wallRunKeyDown, String phase) {
		WallRunState previous = ACTIVE_WALL_RUNS.get(player);
		String logPhase = phase + "_walljump_takeover";
		if (!wallRunKeyDown) {
			logWallRunState(logPhase, "no_wallrun_input", player, playerPatch, previous, wallRunKeyDown);
			return false;
		}
		if (!canAttemptWallRun(player, playerPatch)) {
			logWallRunState(logPhase, "cannot_attempt", player, playerPatch, previous, wallRunKeyDown);
			return false;
		}
		if (!(playerPatch instanceof LocalPlayerPatch localPlayerPatch)) {
			logWallRunState(logPhase, "missing_local_patch", player, playerPatch, previous, wallRunKeyDown);
			return false;
		}

		DecisionResult decisionResult = resolveWallRunDecision(player);
		if (decisionResult == null) {
			logWallRunState(logPhase, "no_wall", player, playerPatch, previous, wallRunKeyDown);
			return false;
		}
		if (!canUseDecisionFromInput(player, decisionResult.decision())) {
			logWallRunState(logPhase, "input_rejected", player, playerPatch, previous, wallRunKeyDown);
			return false;
		}

		boolean jumpHeld = previous == null ? isJumpKeyDown() : previous.jumpHeld();
		finishParCoolWallJumpForWallRun(player, localPlayerPatch);
		applyWallRun(player, localPlayerPatch, decisionResult.decision(), jumpHeld);
		WallRunState next = nextWallRunState(player, null, decisionResult, jumpHeld);
		ACTIVE_WALL_RUNS.put(player, next);
		logDecisionChange(player, previous == null ? null : previous.decision(), decisionResult.decision());
		logWallRunState(logPhase, "parcool_walljump_takeover", player, playerPatch, next, wallRunKeyDown);
		NaturalSprinterFastRunHandler.cancelManualFastRunStepKey(player);
		return true;
	}
	...
	private static void triggerWallBackflip(Player player, LocalPlayerPatch playerPatch, WallRunDecision decision) {
		...
		Vec3 beforeImpulse = player.getDeltaMovement();
		Vec3 impulse = calculateWallBackflipImpulse(player, decision, beforeImpulse);
		player.setDeltaMovement(impulse.x(), impulse.y(), impulse.z());
		player.hasImpulse = true;
		logWallBackflipImpulse(player, decision, beforeImpulse, impulse);
		...
		float xRot = player.isCrouching() ? 0.0F : -20.0F;
	...
	private static Vec3 calculateWallBackflipImpulse(Player player, WallRunDecision decision, Vec3 previousMotion) {
		Vec3 awayFromWall = horizontalUnit(decision.wallDirection().scale(-1.0D), horizontalLook(player).scale(-1.0D));
		Vec3 runDirection = horizontalUnit(decision.runDirection(), rightDirection(awayFromWall));
		Vec3 preservedMotion = new Vec3(previousMotion.x(), 0.0D, previousMotion.z()).scale(WALL_BACKFLIP_PRESERVED_MOMENTUM);
		Vec3 horizontalMotion = awayFromWall.scale(WALL_BACKFLIP_AWAY_SPEED)
				.add(runDirection.scale(WALL_BACKFLIP_RUN_SPEED))
				.add(preservedMotion);
		horizontalMotion = limitHorizontalSpeed(horizontalMotion.x(), horizontalMotion.z(), WALL_BACKFLIP_MAX_HORIZONTAL_SPEED);
		return new Vec3(horizontalMotion.x(), WALL_BACKFLIP_VERTICAL_SPEED, horizontalMotion.z());
	}

	private static Vec3 horizontalUnit(Vec3 vector, Vec3 fallback) {
	...
	private static boolean isWomWallRunAnimation(PlayerPatch<?> playerPatch) {
		return WomSpiderWallMovementState.isWallMovementAnimation(playerPatch);
	}
	...
	private static void finishParCoolWallJumpForWallRun(Player player, LocalPlayerPatch playerPatch) {
		finishParCoolWallJumpAction(player);
		clearParCoolAnimator(player);
		ParCoolWallJumpHandoffState.clear(player);
		stopEpicParCoolWallJumpAnimations(playerPatch);
	}

	private static void finishParCoolWallJumpAction(Player player) {
		try {
			Parkourability parkourability = Parkourability.get(player);
			Class<?> wallJumpClass = parCoolActionClass("com.alrex.parcool.common.action.impl.WallJump");
			Object wallJump = parkourability == null || wallJumpClass == null ? null : parCoolAction(parkourability, wallJumpClass);
			if (isParCoolActionDoing(wallJump)) {
				wallJump.getClass().getMethod("finish", Player.class).invoke(wallJump, player);
			}
	...
	private static void stopEpicParCoolWallJumpAnimations(LocalPlayerPatch playerPatch) {
		WomSpiderWallMovementState.stopPlaying(playerPatch, WomAnimationRefs.epicParCoolWallJumpLeftStart());
		WomSpiderWallMovementState.stopPlaying(playerPatch, WomAnimationRefs.epicParCoolWallJumpRightStart());
		WomSpiderWallMovementState.stopPlaying(playerPatch, WomAnimationRefs.epicParCoolWallJumpLeft());
		WomSpiderWallMovementState.stopPlaying(playerPatch, WomAnimationRefs.epicParCoolWallJumpRight());
	}
	...
	private static void stopWallRunAnimationsOnly(LocalPlayerPatch playerPatch) {
		WomSpiderWallMovementState.stopWallMovementAnimations(playerPatch);
	}
	...
	private static void logDecisionChange(Player player, WallRunDecision previous, WallRunDecision decision) {
		if (!EPMConfig.debugSpiderWallRunState() || !EPM.LOGGER.isDebugEnabled()) {
			return;
		}
	...
	private static void logWallBackflipImpulse(Player player, WallRunDecision decision, Vec3 beforeImpulse, Vec3 impulse) {
		if (!EPMConfig.debugSpiderWallRunState() || player == null) {
			return;
		}

		EPM.LOGGER.info("[EPM/SpiderWallRun] phase=wall_backflip_impulse tick={} mode={} wallDirection={} runDirection={} block={} crouching={} beforeDelta={} impulse={} afterDelta={} pos={}",
				Integer.valueOf(player.tickCount),
				decision.mode(),
				decision.wallDirection(),
				decision.runDirection(),
