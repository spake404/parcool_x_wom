...
public final class MomentumAirAttackWindowState {
	...
	public static void markWallJump(Player player) {
		if (player == null || !ParcoolXWomConfig.wallJumpPrimesAirAttack() || player.onGround() || player.isSpectator() || player.isInWater()) {
			return;
		}
	...
	public static void tick(TickEvent.PlayerTickEvent event) {
		...
		if (ticks.intValue() <= 0 || player.onGround() || player.isSpectator() || player.isInWater() || !ParcoolXWomConfig.wallJumpPrimesAirAttack()) {
	...
	public static boolean shouldProtectWallJumpFall(Player player, float damage) {
		...
		return damage < ParcoolXWomConfig.wallJumpAirAttackFallProtectionDamageThreshold();
	}
