...
import dev.spake404.parcoolxwom.mixin.AnimatorAccessor;
...
public final class ParCoolXWomEvents {
	...
	@SubscribeEvent(priority = EventPriority.LOWEST)
	public static void replaceEpicParCoolCatLeap(ParCoolActionEvent.StartEvent event) {
	...
	public static void markEpicParCoolWallJumpForPhantomAscent(ParCoolActionEvent.StartEvent event) {
		if (ParCoolXWomConfig.wallJumpPrimesPhantomAscent() && "com.alrex.parcool.common.action.impl.WallJump".equals(event.getAction().getClass().getName())) {
	...
	private static void triggerNaturalSprinterDashOnFastRunStart(PlayerPatch<?> playerPatch) {
		...
		if (!shouldTriggerNaturalSprinterDash(playerPatch, wasFastRunActive)) {
			return;
		}

		if (!playerPatch.hasStamina(2.0F)) {
			return;
		}
	...
	private static boolean shouldTriggerManualFastRunDash(PlayerPatch<?> playerPatch) {
		...
		if (Boolean.TRUE.equals(MANUAL_FAST_RUN_KEY_CONSUMED.put(playerPatch, Boolean.TRUE))) {
			return false;
		}
	...
	private static String categoryName(Object category) {
		if (category instanceof Enum<?> enumCategory) {
